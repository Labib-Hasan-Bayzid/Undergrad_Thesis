...
export class PqcBridgeService {
  ...
  //
async deleteModelFile(modelFileId: string): Promise<void> {
  const cmd = `
    cd ${this.shEscape(this.wslProjectDir)} &&
    source .venv/bin/activate &&
    export PGHOST=172.23.0.1 &&
    export PGPORT=5432 &&
    export PGDATABASE=pqc_vault &&
    export PGUSER=postgres &&
    export PGPASSWORD='refat' &&
    export DB_DSN='postgresql://postgres:refat@172.23.0.1:5432/pqc_vault' &&
    export PG_DSN="$DB_DSN" &&
    python bridge_delete.py --file-id ${this.shEscape(modelFileId)}
  `;
  ...
  try {
    ...
    if (!parsed?.ok) {
      throw new Error('Delete bridge did not confirm success');
    }
  } catch (e: any) {
    throw new InternalServerErrorException(`PQC delete failed: ${e.message}`);
  }
}
  ...
  async downloadToWindowsDir(modelFileId: string, windowsOutDir: string): Promise<{ outputPath: string; filename: string }> {
    ...
    try {
      ...
      const windowsPath = parsed.outputPath
        .replace(/^\/mnt\/([a-z])\//, (_, d) => `${d.toUpperCase()}:\\`)
        .replace(/\//g, '\\');

      return {
        outputPath: windowsPath,
        filename: parsed.filename || 'card-details.txt',
      };
    } catch (e: any) {
