...
def main() -> None:
    ...
    ap.add_argument("--out", required=True, help="Output folder or file path")
    args = ap.parse_args()

    if not CA_FILE:
        raise RuntimeError("TLS_CA_FILE not set")
    if not CLIENT_CERT or not CLIENT_KEY:
        raise RuntimeError("TLS_CLIENT_CERT and TLS_CLIENT_KEY must be set")
    ...
    client_send_hello(tls, client_name="bridge_download", features=["mtls", "cap_token_v1", "ed25519_reqsig_v1"])
    client_expect_server_hello(tls)

    auth_req = {
        "client_pub_b64": b64u_enc(client_pub_raw),
        "scope": {"files": [args.file_id]},
    }
    ...
    req = _mk_authorized_request(args.file_id, token_str, client_pub_raw, sk)
    ...
    filename = meta.get("filename") or "decrypted_output.txt"
    out_path = _write_path(args.out, filename)
    ...
    print(json.dumps({"ok": True, "outputPath": str(out_path), "filename": filename}))
